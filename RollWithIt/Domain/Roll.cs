...
namespace RollWithIt.Domain
{
    public class Roll
    {
        ...
        {
            const string Pattern = @"(?<rollOperator>-|\+)?(?:(?<numberOfDice>\d+)(?:d)(?<numberOfSides>\d+)(?:(?<modifierOperator>-|\+)(?<modifier>\d+(?!d(?!l|h))))?(?<diceSubset>(?:(?:dl|dh|kl|kh|rl|rh)(?:\d+))*))+";
            MatchCollection x = Regex.Matches(rollString, Pattern);
            return new Roll();
        }
